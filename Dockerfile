...
COPY ./frontend/static/mediapipe ./static/mediapipe
...
RUN openssl req -x509 -newkey rsa:4096 -nodes \
    ...
    -subj "/C=ES/ST=State/L=City/O=App/OU=Dev/CN=localhost" && \
...
RUN cd src && make
COPY --from=svelte-build /app/build /app/dist_static
...
COPY --from=build /app/dist_static /static_assets
COPY --from=build /etc/ssl/certs/ca-certificates.crt /etc/ssl/certs/
COPY --from=build --chown=10001:10001 /app/certs/server.crt /bin/server.crt
COPY --from=build --chown=10001:10001 /app/certs/server.key /bin/server.key
